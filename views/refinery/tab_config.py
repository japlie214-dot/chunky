...
@st.fragment
def _refinery_page_count_fragment(session, sel_file, stage_path, scope):
    ...
    Uses st.session_state directly (not _jbv/_jbsync closures).
    """
    page_count_est = 1
    ...
    if scope == "Page Range":
        ...
        _ps_val = st.session_state.get("_jbv_pstart", 1)
        _pe_val = st.session_state.get("_jbv_pend", 10)
        ...
        if p_start != _ps_val:
            st.session_state["_jbv_pstart"] = p_start
        if p_end != _pe_val:
            st.session_state["_jbv_pend"] = p_end
...
def render_config_tab(session):
    ...
    with st.container():
        ...
        with jc1:
            ...
            page_count_est, p_start, p_end = _refinery_page_count_fragment(
